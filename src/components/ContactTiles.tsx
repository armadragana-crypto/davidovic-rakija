...
/* The distillery's own places. Every link and every delayed opening reads from
   ...
   the front page can never drift apart. */
...
const ContactTiles = forwardRef<HTMLDivElement, { className?: string }>(function ContactTiles(
  { className },
  ref
) {
  ...
  return (
    <div ref={ref} className={className ? `contact-grid ${className}` : 'contact-grid'}>
      <a
        href={PHONE_URL}
        ...
        onClick={openWhenSeen('tel', PHONE_URL)}
      >
        ...
        <span className="contact-tile-value">{PHONE_NUMBER}</span>
      ...
      <a
        ...
        onTouchCancel={cancelPress}
        onClick={openWhenSeen('map', MAPS_URL)}
      >
